...
namespace lslidar_ch_driver {
    ...
    static const size_t ch128s1_packet_size = sizeof(lslidar_msgs::Lslidar128S1Packet().data);
////////////////////////////////////////////////////////////////////////
...
/** @brief destructor */
    ...
    int InputSocket::getPacket(lslidar_msgs::LslidarChPacketPtr &packet) {
        ...
        while (flag == 1) {
            ...
            ssize_t nbytes = recvfrom(sockfd_, &packet->data[0], ch128x1_packet_size, 0,
            ...
            } else if ((size_t) nbytes == ch128x1_packet_size) {
    ...
    int InputSocket::getPacket(lslidar_msgs::Lslidar128S1PacketPtr &packet) {
        ...
        while (flag == 1) {
            ...
            // Receive packets that should now be available from the
            // socket using a blocking read.
            ssize_t nbytes = recvfrom(sockfd_, &packet->data[0], ch128s1_packet_size, 0,
                                      (sockaddr *) &sender_address, &sender_address_len);
            ...
            } else if ((size_t) nbytes == ch128s1_packet_size) {

                // read successful,
                // if packet is not from the lidar scanner we selected by IP,
                // continue otherwise we are done
                if (devip_str_ != "" && sender_address.sin_addr.s_addr != devip_.s_addr){
                    ROS_WARN_THROTTLE(2,"lidar ip param error,please reset in launch file");
                    continue;
                }
                else
                    break; //done
    ...
    int InputPCAP::getPacket(lslidar_msgs::LslidarChPacketPtr &pkt) {
        ...
        while (flag == 1) {
            ...
            if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0) {
                ...
                mempcpy(&pkt->data[0], pkt_data + 42, ch128x1_packet_size);
                empty_ = false;
                return 0;
    ...
    int InputPCAP::getPacket(lslidar_msgs::Lslidar128S1PacketPtr &pkt) {
        struct pcap_pkthdr *header;
        const u_char *pkt_data;
        while (flag == 1) {
            int res;
            if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0) {
                ...
                mempcpy(&pkt->data[0], pkt_data + 42, ch128s1_packet_size);
